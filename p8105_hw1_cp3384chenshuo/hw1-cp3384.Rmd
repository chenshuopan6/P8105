---
title: "HW1_cp3384"
author: "Chenshuo Pan"
output: github_document
---

# Question 1

```{r}
#Load the moderndive library
library(moderndive)
library(ggplot2)
library(tidyverse)
```


```{r}
#load dataset
data("early_january_weather")
```
_Answer_: This is a data set describing the weather at Newark Airport for each time period in January 2013. Specific variables include:`r names(early_january_weather) `

This is a dataset has `r nrow(early_january_weather) ` rows, and `r ncol(early_january_weather) ` columns. The average temperature in January is`r mean(early_january_weather$temp) 


```{r,draw scatter plot}
#make hour vs temp scatter plot
ggplot(data = early_january_weather,mapping = aes(x = time_hour,y = temp,color = humid))+
  geom_point()+
  xlab('Time/hour')+
  ylab('Temperature')+
  ggtitle('Scatter plot of hour vs temp ')

```

```{r,download scatter plot}
ggsave(filename = ("./scatterplotQ1.png"))
```

# Question 2


```{r,create dataframe}
#Set seed to make sure each time we will get same number
set.seed(666)

#Create a random sample of size 10 from a standard Normal distribution

random_sample <- rnorm(10)

#Create a logical vector indicating whether elements of the sample are greater than 0

logical_sample <- random_sample > 0

#Create a character vector of length 10

character_sample <- LETTERS[1:10]

#Create a factor vector of length 10, with 3 different factor “levels”

factor_sample <-as.factor(rep(c("First", "Second", "Third"), length = 10))

#Combine all the above data in one dataframe

Desired_dataframe <- data.frame(
  random_sample,
  logical_sample,
  character_sample,
  factor_sample
)

Desired_dataframe
```


```{r}
# Try to see which variables have mean
# logical and numerical variables have mean
mean(pull(Desired_dataframe,random_sample))

mean(pull(Desired_dataframe,logical_sample))

```
```{r}
# character variable does not have mean
mean(pull(Desired_dataframe,character_sample))
```

```{r}
# factor variable does not have mean
mean(pull(Desired_dataframe,factor_sample))
```

_Answer_: There is a mean in numerical and logical values. In logical values, True is 1 and False is 0. Other variables such as factor and character cannot calculate mean.


```{r,apply as numeric,eval=FALSE}
as.numeric(pull(Desired_dataframe,factor_sample))
as.numeric(pull(Desired_dataframe,character_sample))
as.numeric(pull(Desired_dataframe,logical_sample))
```

_Answer_: The factor variables will be converted to numbers corresponding to the levels. For example, in my data frame, first is converted to 1, second is converted to 2.... This function cannot be used with characters variable. True in the logical variable is converted to 1 and False is converted to 0.This helps me explain why logistic variables have averages, but factor variables don't.





